...
{
  RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Configuring up...please wait...");
...
hardware_interface::CallbackReturn DiffDriveArduinoHardware::on_cleanup(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Cleaning ...please wait...");
  comms_.disconnect();
...
{
  comms_.read_encoder_values(wheel_l.enc,wheel_r.enc);
...
hardware_interface::return_type diffdrive_arduino ::DiffDriveArduinoHardware::write(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  int motor_l_counts_per_loop = wheel_l.cmd / wheel_l.rads_per_count / cfg_.loop_rate;
  int motor_r_counts_per_loop = wheel_r.cmd / wheel_r.rads_per_count / cfg_.loop_rate;
